...
// push() 메서드는 배열의 끝에 하나 이상의 새 요소를 추가하고 새 길이를 반환한다. concat()과 달리 push()는 배열 인자를 평탄화하지 않는다.
// pop() 메서드는 그 반대이다. 배열의 마지막 요소를 꺼내서 반환하며 배열 길이를 줄인다. 두 메서드 모두 기존의 배열을 수정한다.
// push()와 pop()을 조합해 자바스크립트 배열을 선입후출의 스택처럼 사용할 수 있다.
...
// push() 메서드는 전달한 배열을 평탄화하진 않는다. 배열 요소를 다른 배열에 모두 넣고 싶다면 분해 연산자를 직접 사용해 평탄화 할 수 있다.
a.push(...values)

// unshift()와 shift() 메서드는 push(), pop()과 거의 비슷하지만 배열의 마지막이 아니라 앞부분에서 이루어진다는 것이 다르다.
...
// unshift()에 인자 여러 개를 전달하면 이들은 모두 한 번에 삽입되므로 한 번에 하나씩 삽입했을 때와는 결과가 다르다.

// 5.8 하위배열
// 배열에는 일종의 연속적인 영역인 하위 배열, 슬라이스(slice)를 다루는 메서드도 있다.
// 다음 절은 슬라이스를 추출, 대체, 삽입, 복사하는 메서드를 설명한다.

// 5.8.1 slice()
// slice() 메서드는 지정된 배열의 하위 배열을 반환한다. 두 개의 인자는 각각 반활될 슬라이스의 시작과 끝 위치를 나타낸다.
// 반환된 배열에는 첫 번째 인자로 지정된 요소에서 시작해, 두 번째 인자로 지정된 요소 바로 앞까지가 포함된다.
// 인자를 하나만 사용한다면, 반환된 배열은 해당 위치부터 원래 배열의 마지막 요소까지 포함한다.
// 인자의 음수를 사용한다면 그 값에 배열 길이를 더한 값을 적용한다. 예를 들어 -1은 배열의 마지막 요소이고, -2는 마지막에서 바로 앞 요소이다.
// slice는 원래 배열을 수정하지 않는다.
...
// slice()나 concat()과는 달리 splice()는 원래 배열을 수정한다.
// splice()는 배열에서 요소를 삭제하거나 삽입할 수 있고 두 동작을 동시에 할 수도 있다. => 그래서 주로 배열에서 특정 요소를 제거할 때 많이 사용한다.
// 삽입이나 제거가 이루어진 위치 다음에 오는 요소들을 앞이나 뒤로 밀어 배열을 빽빽하게 유지한다.
// splice()의 첫 번째 인자는 삽입이나 제거를 시작할 위치이다. 두 번째 인자는 제거할 요소의 개수이다.

let a = [1, 2, 3, 4, 5, 6, 7, 8]
a.splice(4) // => [5, 6, 7, 8]를 제거하겠다는 뜻. 따라서 a = [1, 2, 3, 4]
a.splice(1, 2) // => [1, 4]
...
let a = new Array(5) // 요소가 없고 길이가 5인 배열로 시작한다.
a.fill(0) // => [0, 0, 0, 0, 0] 배열을 0으로 채운다.
a.fill(9, 1) // => [0, 9, 9, 9, 9] 인덱스 1에서 시작해 9로 채운다.
a.fill(8, 2, -1) // => [0, 9, 8, 8, 9] 인덱스 2에서 시작해 3까지 8로 채운다.

// fill()의 첫 번째 인자는 배열 요소로 사용할 값이다.
// fill()의 두 번째 인자는 선택사항인 시작 인덱스이다.
// fill()의 세 번째 인자는 선택사항인 마지막 인덱스이며, 이 바로 앞까지 작업한다.

// 5.8.4 copyWithin()
// copyWithin()은 배열의 슬라이스를 복사해 새 위치에 붙여 넣는다.
// 이 메서드는 배열을 수정해 반환하지만 길이는 바뀌지 않는다.
// 첫 번째 인자는 첫 번째 요소가 복사될 위치의 인덱스이다.
// 두 번째 인자는 복사할 첫 번째 요소의 인덱스이다. 두 번째 인자를 생각하면 0으로 사용한다.
// 세 번째 인자는 복사할 슬라이스의 끝을 지정한다. (생략하면 배열 길이를 사용한다.)
// 다른 메서드와 마찬가지로 인덱스 바로 앞까지를 복사한다.
// 역시 다른 메서드와 마찬가지로 음수를 사용하면 그 값을 배열 길이에 더해 적용한다.

let a = [1, 2, 3, 4, 5]
a.copyWithin(1) // => [1, 1, 2, 3, 4] // 전체를 복사해 인덱스 1에 덮어 쓴다.
a.copyWithin(2, 3, 5) // => [1, 1, 3, 4, 4] // 마지막 두 개를 복사해 인덱스 2에 덮어 쓴다.
...
// indexOf()와 lastIndexOf()는 지정된 값을 배열에서 찾아 그 첫 번째 요소의 인덱스를 반환하며, 찾지 못하면 -1을 반환한다.
// indexOf()는 배열의 앞에서부터 검색을 시작하고 lastIndexOf()는 배열의 뒤에서부터 역순으로 검색한다.

let a = [0, 1, 2, 1, 0]
a.indexOf(1) // => 1 : a[1]은 1이다.
a.lastIndexOf(1) // => 3 : a[3]은 1이다.
a.indexOf(3) // -1 : 값이 3인 요소는 없다.

// indexOf()와 lastIndexOf()는 인자와 배열 요소를 비교할 때 === 연산자를 사용한다.
// 배열에 기본 값이 아니라 객체가 들어 있다면 이들 메서드는 두 참조가 정확히 같은 객체를 참조하는지 확인한다.
// 객체 콘텐츠를 검색하려는 목적이라면 find() 메서드와 판별 함수를 사용하자.

// includes()
// includes() 메서드는 ES2016에서 도입한 메서드이다.
// 인자 하나를 받고 배열에 그 값이 포함되어 있으면 true를, 그렇지 않다면 false를 반환한다.
// 이 메서드는 인덱스를 반환하지 않으며 그런 값이 있는지 없는지 여부만 반환한다. true or false

// sort()
// sort()는 배열 요소를 정렬한다. 인자 없이 sort()를 호출하면 배열 요소를 알파벳순으로 정렬한다.
...
// 이 메서드는 새 배열을 만들지 않고 기존 배열의 요소 순서를 뒤집는다.

let a = [1, 2, 3]
a.reverse() // => [3, 2, 1]

// 5.8.6 배열을 문자열로 변환
// Array 클래스에는 배열을 문자열로 변환하는 메서드가 세 가지 있는데 일반적으로 로그나 에러 메시지를 만들 때 이 메서드를 사용한다.
// join() 메서드는 배열 요소 전체를 문자열로 반환한 다음, 이들을 병합합 결과를 반환한다.
// 결과 문자열에서 각 요소를 구분하는 문자열을 선택사항으로 지정할 수도 있다. 구분자를 지정하지 않으면 콤마를 사용한다.
...
// join() 메서드는 문자열을 분할하는 String.split() 메서드의 역입니다.

// 5.8.7 정적 배열 함수
// Array 클래스에는 Array 생성자를 사용해 호출할 수 있는 정적 함수가 세 가지 있다.
// Array.of()와 Array.from()은 새 배열을 만드는 팩토리 메서드이다.
